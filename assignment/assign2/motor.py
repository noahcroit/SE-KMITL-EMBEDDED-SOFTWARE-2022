...
class PCA9685:
    ...
    def __init__(self,i2c_num=0,i2c_scl=9,i2c_sda=8,i2c_freq=100_000,address=0x40,debug=False):
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("motor A forward, speed 25%, Run for 1S, then stop")
        m.MotorRun('MA', 'forward', 25, 1)
        # print("motor A backward, speed 100%, Run for 2S, then stop")
        # m.MotorRun('MA', 'backward', 100, 2)
        print("motor B forward, speed 50%, Run for 1S, then stop")
        m.MotorRun('MB', 'forward', 50, 1)
        #print("motor B forward, speed 50%, Run for 4S, then stop")
        #m.MotorRun('MB', 'backward', 50, 4)

        print("motor C forward, speed 75%, Run for 1S, then stop")
        m.MotorRun('MC', 'forward', 75, 1)
